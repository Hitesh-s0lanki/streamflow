...
import type { MovieMatch, MovieSearchResult } from "./use-movie-generation";

interface MovieSelectionSectionProps {
  searchResults: MovieSearchResult;
  onSelect: (movie: MovieMatch) => void;
  onBack: () => void;
}

export function MovieSelectionSection({
  searchResults,
  onSelect,
  onBack,
}: MovieSelectionSectionProps) {
  return (
    <motion.div
    ...
    >
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-xs">
              Select the correct movie to continue
            </p>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {searchResults.matches.map((movie, index) => (
            <motion.button
